...
//-----------------------------------------------------------------------------------------------------------------------------
{
	...
	for (int bounces = 0; bounces < 6; bounces++)
	{
                ...
                if (hitType == DIFF) // Ideal DIFFUSE reflection
		{
			...
			dirToLight = randomDirectionInSpecularLobe(normalize(lightChoice.position - x), nl, 0.15);
		...
		if (hitType == COAT)  // Diffuse object underneath with ClearCoat on top
		{
			...
			dirToLight = randomDirectionInSpecularLobe(normalize(lightChoice.position - x), nl, 0.2);
